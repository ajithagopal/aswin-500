...
let chartContainer;
let chartOffset;
var chartPos = getOffset(document.getElementById('chart'))
...
function startVisualization() {
	return d3.csv("./assets/charts/chart-1/bowler_dataNew.csv").then(data => { // Return the promise here
...
document.getElementById('reset-button').addEventListener('click', function() {
    ...
    document.getElementById('milestone-messages').style.display = "none"
});
...
function updateChart(matchData) {
    ...
    function calculateViewportHeight(wickets) {
        ...
        console.log('step 1 ',yPixelValue + pixelOffset, ' --- ', maxScroll)
    ...
    function displayMilestoneMessages() {
        ...
        Object.entries(milestonesByMatch).forEach(([matchKey, bowlers]) => {
            ...
            if(matchKey.includes(`${currentMilestone} Wickets`)) {
                ...
                document.getElementById('milestone-messages').style.display = "block"
            }
        });
    }
    ...
    matchData.forEach(d => {
        ...
        if (racePaused) {
            ...
            console.log('step 2 ', viewportHeight)
            window.scrollTo({
                top: chartPos.top + viewportHeight + chartOffset + 150,
                behavior: 'smooth'
            });
        }
	...
	let winner = matchData.find(d => d.wicketsTaken >= maxWickets);
		if (winner) {
            ...
            console.log('step 3 ', viewportHeight)
            window.scrollTo({ 
                top: chartPos.top + viewportHeight + chartOffset + 300,
                behavior: 'smooth'
            });
        }
